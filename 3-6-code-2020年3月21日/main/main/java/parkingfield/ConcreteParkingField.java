...
public class ConcreteParkingField implements ParkingField {
	...
	//停车记录
	//为提升检索性能，后续可以考虑分为当前在停车场车辆记录和历史记录两个Set
	...
	@Override
	public double depart(String plateNo) throws CarNotInFieldException {
		Car car= getCarByNo(plateNo);
		if (!status.containsValue(car) || car==null)
			throw new CarNotInFieldException(plateNo);

		double fee=-1;
		Lot toBeemptiedLot=null;
		for(Lot lot:status.keySet()) { //找到需要从status中删除的lot
		...
		for(Record record:records) {
			if(record.getCar().equals(car)&&record.getLot().equals(toBeemptiedLot)
